                    ...
                    if($stmt->rowCount() > 0) {
                            ...
                            while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
                                    ...
                                    if($id_get != $_SESSION['user_id']){
                                        $can_delete = "<button class='btn btn-sm m-1 delete' id={$id_cript_to_del} email-data={$email} data-toggle='tooltip' data-placement='top' title='Deletar usuário'><i class='fas fa-trash'></i></button>";
                                    }
